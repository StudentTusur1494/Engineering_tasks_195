...
# Полоса пропускания
delta_f = f0 / Q                       # через Q
delta_f_check = R / (2 * np.pi * L)    # через R и L (проверка)

# Точные граничные частоты по уровню -3 дБ
f1 = f0 * (np.sqrt(1 + 1/(4*Q**2)) - 1/(2*Q))
f2 = f0 * (np.sqrt(1 + 1/(4*Q**2)) + 1/(2*Q))
...
# Фаза в градусах (ФЧХ)
H_phase = np.angle(H_complex, deg=True)
...
plt.subplot(3, 1, 1)
plt.plot(f, H_mag, 'b-', linewidth=2)
plt.axvline(f0, color='r', linestyle='--', alpha=0.7, label=f'f₀ = {f0:.0f} Гц')
plt.axhline(1/np.sqrt(2), color='gray', linestyle=':', alpha=0.7, label='Уровень -3 дБ (0.707)')
...
plt.axvline(f0, color='r', linestyle='--', alpha=0.7, label=f'f₀ = {f0:.0f} Гц')
plt.axhline(-3, color='gray', linestyle=':', alpha=0.7, label='Уровень -3 дБ')
plt.xlabel('Частота, Гц')
plt.ylabel('|H(f)|, дБ')
...
# -------- ФЧХ --------
plt.subplot(3, 1, 3)
plt.semilogx(f, H_phase, 'b-', linewidth=2)
plt.axvline(f0, color='r', linestyle='--', alpha=0.7, label=f'f₀ = {f0:.0f} Гц')
